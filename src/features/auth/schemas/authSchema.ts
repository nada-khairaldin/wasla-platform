...
export const basicInfoSchema = z
  .object({
    ...
    confirmPassword: z.string().min(8, "الرجاء تأكيد كلمة المرور"),
  })
  .refine((data) => data.password === data.confirmPassword, {
    message: "كلمتا المرور غير متطابقتين",
    path: ["confirmPassword"], // to show the error message under the confirmPassword field not the form level
  });

  export const skillsSchema = z.object({
  offeredSkills: z.array(z.string()).min(1, "يرجى اختيار مهارة واحدة على الأقل تقدمها").max(5, "يرجى اختيار خمس مهارات كحد أقصى"),
  requiredSkills: z.array(z.string()).min(1, "يرجى اختيار مهارة واحدة على الأقل تحتاجها").max(5, "يرجى اختيار خمس مهارات كحد أقصى"),
});
