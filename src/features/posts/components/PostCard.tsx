...
const PostCardComponent = ({
  ...
  } = useUserActions(post.userId || post.user?.id);
  ...
  const isMyPostsView =
    pathname === "/my-posts" || pathname?.startsWith("/my-posts/");
    ...
    if (hasConversation && conversationId) {
      ...
      try {
        const conversation = await createConversation.mutateAsync({
          postId: post.id,
          postOwnerId: post.userId || post.user?.id || 0,
        });
  ...
  return (
    <div
      ...
      className={`relative flex flex-col p-5 md:p-6 rounded-xl transition-all duration-500 cursor-pointer ${isRecommended ? "h-full" : ""}
      hover:[&_.identification-arrow]:-translate-x-1.25 hover:[&_.card-title]:text-primary-700
      ${
        isHomeView
          ? "bg-neutral-50"
          : isSaved || isRecommended || isMyPostsView
            ? "bg-white"
            : "bg-neutral-50"
      } ${
        isRecommended
          ? "border-2 border-secondary-400 shadow-[0_10px_35px_rgb(239,207,133,0.15)] hover:shadow-secondary-300/40 bg-white"
          : isHomeView
    ...
    >
      ...
      <div className="flex gap-sm items-center mb-4 md:mb-5">
        {/* Mobile: show 3-dot menu only if NOT the post owner in home view; otherwise show direct save button */}
        <div className="flex md:hidden relative z-20">
          {isHomeView && isSelf ? null : isHomeView &&
            !isSelf &&
            !isUserLoading ? (
            <button
              onClick={handleSave}
              className={`w-9 h-9 flex items-center justify-center rounded-full transition-all duration-300 shadow-sm ${isSaved ? "bg-primary-600 text-white" : "bg-white text-primary-300 border border-neutral-100"}`}
            >
          ...
          >
            ...
            >
              ...
              {!isSelf && !isUserLoading && (
                ...
                >
                  <div
                    className={`p-2 rounded-lg ${isSaved ? "bg-primary-50 text-primary-600" : "bg-neutral-100 text-neutral-500"}`}
                  >
                    <Bookmark
                      size={20}
                  ...
                  <span
                    className={`font-bold font-cairo text-base ${isSaved ? "text-primary-700" : "text-neutral-700"}`}
                  >
                    {isSaved ? "إلغاء الحفظ" : "حفظ المنشور"}
                  </span>
                </button>
              ...
              {onEdit && (
                ...
                >
                  ...
                  <span className="font-bold font-cairo text-base text-neutral-700">
                    تعديل المنشور
                  </span>
                </button>
              )}
              ...
              {onDelete && (
                ...
                >
                  ...
                  <span className="font-bold font-cairo text-base text-error-600">
                    حذف المنشور
                  </span>
                </button>
              )}
